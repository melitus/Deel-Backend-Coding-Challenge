...
class DeelStackError extends Error {
...
class AppError extends DeelStackError {
  /**
...
class PaymentError extends DeelStackError {
...
class DepositError extends DeelStackError {
  constructor(status = httpStatus.UNAUTHORIZED, title = 'Authentication error', message = 'Invalid Credentials.') {
    super(message);
    this.status = status;
    this.title = title;
...
class NotFoundError extends DeelStackError {
  constructor(
...
function isTrustedError(error) {
  if (error instanceof DeelStackError) {
